...
namespace Basket.Domain.Shared
{
    /// <summary>
    ...
    public static class Guard
    {
        ...
        {
            That<Exception>(condition, code, message);
        }
        ...
        public static void That<TException>(bool condition, string code = "", string message = "",
            string userFriendlyMessage = "") where TException : Exception
        {
            if (condition)
            {
                throw !string.IsNullOrEmpty(userFriendlyMessage)
                    ? Activator.CreateInstance(typeof(TException), code, message, userFriendlyMessage) as TException
                    : Activator.CreateInstance(typeof(TException), code, message) as TException;
            }
        }
